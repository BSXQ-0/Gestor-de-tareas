...
function Login() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            setError(error.response?.data?.message || 'Error en el login. Verifica tus credenciales.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="bg-background-light dark:bg-background-dark min-h-screen relative overflow-hidden flex flex-col items-center justify-center whiteboard-texture">
            ...
            <main className="flex flex-col items-center justify-center p-4 w-full max-w-4xl">
                ...
                <div className="tilted-card relative bg-white dark:bg-zinc-900 p-8 md:p-12 w-full max-w-[480px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-xl border border-slate-200 dark:border-zinc-800">
                    {/* Pin Head */}
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 size-8 bg-red-500 rounded-full shadow-inner flex items-center justify-center">
                        <div className="size-2 bg-red-300 rounded-full opacity-50"></div>
                    </div>
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        <div className="flex flex-col">
                            <div className="flex justify-between items-center mb-2">
                                <label className="text-[#0c141d] dark:text-slate-200 text-sm font-bold ml-1">
                                    Contraseña
                                </label>
                        ...
                        {/* Submit Button */}
                        <button
                            className="w-full h-14 bg-primary text-white font-bold text-lg rounded-lg shadow-lg hover:shadow-primary/30 hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                            type="submit"
                            disabled={isLoading}
                        >
                            <span className="truncate">
                                {isLoading ? 'Iniciando sesión...' : 'Iniciar sesión en el panel'}
                            </span>
                            {!isLoading && (
                    ...
                    {/* Register Link */}
                    <div className="mt-10 pt-8 border-t border-slate-100 dark:border-zinc-800 text-center">
